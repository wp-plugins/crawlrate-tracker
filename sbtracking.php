	...
	*/
	class b3_sbTracking extends b3_sbTrackingConfig
	{
		...
		function add_button($content) {
			...
			{
				global $post;
				$content .= "<p><a href='".get_bloginfo('wpurl').'/wp-admin/index.php?page=search-bot-report&page_url='.get_permalink($post->ID)."'><img width='16' src='".$this->sbtracking_plugin_dir."stats.jpg' alt='Bot statistics for this page' title='Bot statistics for this page'/></a></p>";
			}
	...
	class sbTrackingAdmin extends b3_sbTrackingConfig
	{
		...
		private function tracking_bot_report_text()
		{
			...
			if ($url = $_GET['page_url'])
			{
				...
				$total_days = $this->wpdb->get_var("SELECT COUNT(DISTINCT(DATE(FROM_UNIXTIME(`visit_time`)))) as counter FROM wp_sbtracking WHERE `page_url` = '$page_url' AND `visit_time` >= '$start' AND `visit_time` <= '$end'");
...
#				echo "SELECT `visit_time`, `robot_name` FROM $this->sbtracking_table WHERE `page_url` = '$page_url' ORDER BY `visit_time` DESC LIMIT $skip, $page_size";
		...
		public function tracking_crawled_pages(){
			...
			if($_GET['robot'] != '')
			{
				$robot = "AND robot_name = '" . $_GET['robot'] . "'";
			}
